...
function filterItems() {
    ...
    items.forEach(item => {
        ...
        var matchClassification = Array.from(checkedClassification).some(checkbox => itemCategory === checkbox.id)
        var matchDeviceType = Array.from(checkedDeviceType).some(checkbox => itemDeviceType === checkbox.id)
    ...
    if (checkedClassification.length === 0 && checkedDeviceType.length === 0){
        clearFilters()
    }
}
